...
// Data for custom predictor
int phistoryBits; // Number of bits used for Global History for perceptron
int32_t **weight;
int y;
int num_entries;
int8_t *phistory;
...
void custom_init(){
  phistoryBits = 60;
    ...
    //ghistory = 0 & gmask;
    phistory = (int8_t*)malloc(phistoryBits * sizeof(int8_t));
    weight = (int32_t**)malloc(num_entries * sizeof(int32_t*));
    for (int i = 0; i < phistoryBits; i++)
      phistory[i] = 0;
    for (int i = 0; i < num_entries; i++)
      weight[i] = (int32_t*)malloc((phistoryBits+1) * sizeof(int32_t));
    for (int i = 0; i < num_entries; i++)
      for (int j = 0; j < phistoryBits+1; j++)
        weight[i][j] = 0;
...
uint8_t custom_predict(uint32_t pc){
  uint8_t res;
  uint32_t entry = pc % num_entries;
   y = 0;
   for (int i = 0; i < phistoryBits; i++)
     y += weight[entry][i] * phistory[i];
   y += weight[entry][phistoryBits];
   res = (y >= 0) ? TAKEN: NOTTAKEN;
   return res;
}

void custom_train(uint32_t pc, uint8_t outcome){
  int8_t t = outcome ? 1 : -1;
    uint32_t threshold = (uint32_t) (1.93 * phistoryBits + 14);
    uint32_t entry = pc % num_entries;
    if (y*t <= 0 || abs(y) < threshold) {
      for (int i = 0; i < phistoryBits; i++)
        weight[entry][i] += t * phistory[i];
      weight[entry][phistoryBits] += t;
    }
    for (int i = 0; i < phistoryBits-1; i++)
      phistory[i] = phistory[i+1];
    phistory[phistoryBits-1] = t;
}
...
void perceptron_shift(int8_t* satuate, uint8_t outcome){
  if(outcome == TAKEN){
    if(*satuate != (1 << (p_PCLEN / 2) - 1)){
      (*satuate)++;
    }
  }else{
    if(*satuate != -(1 << (p_PCLEN / 2) )){
      (*satuate)--;
    }
  }
}
...
void neural_shift(int32_t* satuate, uint8_t outcome){
  if(outcome == TAKEN){
    if(*satuate != (1 << (n_PCLEN / 2) - 1)){
      (*satuate)++;
    }
  }else{
    if(*satuate != -(1 << (n_PCLEN / 2) )){
      (*satuate)--;
    }
  }
}
...
void neural_path_init(){
  //printf("neural: PC_len: %d\theight: %d\tsatuate_len: %d\n", n_PCLEN, n_HISTORYLEN, n_SATUATELEN);
